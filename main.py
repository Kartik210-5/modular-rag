...
def setup_pipeline(pdf_folder: str = "./pdfs"):
    ...
    if vector_store.collection.count() == 0:
        print("[Setup] Vector store empty. Processing PDFs from './pdfs'...")
...
def main():
    ...
    while True:
        try:
            ...
            # 2. Retrieve, decompose, and re-rank context (Receives 3 returned values)
            context, sources, sub_queries = retriever.get_context_and_sources(
                search_query, top_k_parents=2
            )

            # 3. Pass query, context, and history to LLM
            ...
            print(answer)
            print("-" * 50)
            print(f"Sub-Queries Run: {sub_queries}")
